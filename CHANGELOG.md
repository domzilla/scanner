...
- `--jpeg-quality <0-100>` option to control the MRC background JPEG quality (default: 70, higher = better quality, larger file). Non-numeric or out-of-range values fall back to the default. Only affects MRC PDF output — has no effect on `--format jpeg` or on rotated output.
- Raised the default MRC background JPEG quality from an effective hardcoded 50 (0.5) to 70, since 50 was visibly too rough for typical documents. Users who want the previous, smaller-file behavior can pass `--jpeg-quality 50`.
