...
- `OutputProcessor` — Post-processing pipeline: OCR (Vision), AI summarization/auto-naming (FoundationModels), rotation (CoreImage), PDF combining (PDFKit), file output with tag-based aliasing
...
- Made `ScanConfiguration` immutable (`Sendable`-conformant) — autoname result stored on `OutputProcessor` instead of mutating config
